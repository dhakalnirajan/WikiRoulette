...
import { useArticleFacts } from "@/composables/useArticleFacts";

const props = defineProps<{
  processedHtml: string;
}>();

const { facts, loading, error, refresh } = useArticleFacts();
...
<template>
  <div class="article-facts" @mouseenter="pause" @mouseleave="resume">
    ...
    <div v-if="loading" class="facts-loading">
      <span>Extracting interesting facts...</span>
    </div>
    <div v-else-if="error" class="facts-error">
      <span>{{ error }}</span>
    </div>
    <div v-else-if="facts.length === 0" class="facts-empty">
    ...
    <div v-else class="facts-carousel">
      ...
      <div class="fact-content">
        <p class="fact-text">{{ facts[activeIndex].text }}</p>
      </div>
